...
public class FXMLContactosController implements Initializable {
    ...
    public void aceptarOnClick(ActionEvent e) {
        if (!lbl_cumple.isVisible()) {
            ...
            if (!camposIncompletos()) {
                Contacto nc;
                nc = new Contacto(contactos.get(contactList.getSelectionModel().getSelectedIndex()).getIdContacto(),
                        txt_nombre.getText(), txt_telefono.getText(), txt_email.getText(), txt_direccion.getText(),
                        txt_apodo.getText(),
                        Date.from(txt_date.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant()));
